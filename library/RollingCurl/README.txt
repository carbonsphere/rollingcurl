...
### Updated with source index to allow RollingCurlRequest to attach a track id which can be later used in response processing.

```php
$request = new RollingCurlRequest($url, '', '', '','', $source_index)


request_callback($response, $info, $extra) {
